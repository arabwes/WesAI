"""Toast API OAuth client — SYNCHRONIZED MIRROR of shibam-marketing-mcp/clients/toast_client.py.
...
class ToastAuthError(RuntimeError):
    ...
    as curated setup guidance that is safe to show the caller."""
    _no_retry = True
    _user_facing = True
...
def _auth_error(path: str, body: str) -> "ToastAuthError":
    # Full detail (including the upstream response body) is logged server-side
    # only; the returned message contains no GUIDs or raw upstream content.
    logger.error("Toast API 401 on %s (restaurant_guid set=%s, env=%s). Raw response: %s",
                 path, bool(config.toast_restaurant_guid), config.toast_environment, body)
    ...
    return ToastAuthError(
        ...
        "  1. TOAST_RESTAURANT_GUID does not "
