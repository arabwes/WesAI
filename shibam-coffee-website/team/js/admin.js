...
(function () {
  ...
  function renderPositionOptions(mount, selected) {
    ...
    adminPositions.forEach(function (position) {
      ...
      input.checked = selected.indexOf(position.id) !== -1;
      label.appendChild(input); label.appendChild(document.createTextNode(' ' + position.name)); mount.appendChild(label);
    });
  }
  ...
  function buildUserRow(user, refresh) {
    ...
    var editBtn = el('button', 'btn-inline-action', 'Edit');
    editBtn.type = 'button';
    editBtn.addEventListener('click', function () { editUser(user, refresh); });
    actionCell.appendChild(editBtn);
  ...
  function editUser(user, refresh) {
    ...
    if (!dialog) {
      ...
      dialog.querySelector('form').addEventListener('submit', function (event) {
        ...
        Auth.apiCall('updateManagedUser', { user: { id: form.id.value, name: form.name.value, email: form.email.value, role: form.role.value, maxWeeklyMinutes: Math.round(Number(form.maxHours.value) * 60), active: form.active.checked, positionIds: selectedPositions(form.querySelector('[data-position-options]')) } }).then(function (result) {
          if (result.ok) { dialog.close(); if (form._refresh) form._refresh(); }
          else setStatus(form.querySelector('[data-form-status]'), 'error', Auth.errorMessage(result, 'Could not update employee.'));
        });
      });
    }
    var form = dialog.querySelector('form'); form._refresh = refresh; form.id.value = user.id; form.name.value = user.name; form.email.value = user.email || ''; form.role.value = user.role; form.maxHours.value = user.maxWeeklyMinutes / 60; form.active.checked = user.active;
    renderPositionOptions(form.querySelector('[data-position-options]'), user.positionIds || []); setStatus(form.querySelector('[data-form-status]'), null, ''); dialog.showModal();
  }
  ...
  function loadInvitations() {
    var mount = document.getElementById('invitation-admin-list'); if (!mount) return; mount.textContent = 'Loading…';
